    ...
    fn process_event<A: ApplicationHandler>(
        ...
        match event_option {
            ...
            EventOption::Mouse(MouseEvent { x, y }) => {
                app.window_event(window_target, window_id, event::WindowEvent::CursorMoved {
                    device_id: None,
                    position: (x, y).into(),
                });
            },
            ...
            EventOption::Button(ButtonEvent { left, middle, right }) => {
                while let Some((button, state)) = event_state.mouse(left, middle, right) {
                    app.window_event(window_target, window_id, event::WindowEvent::MouseInput {
                        device_id: None,
                        state,
                        button,
                    });
    ...
    pub fn run_app<A: ApplicationHandler>(mut self, mut app: A) -> Result<(), EventLoopError> {
        ...
        loop {
            ...
            while let Some(window) = {
                ...
                let window_id = WindowId::from_raw(window.fd as usize);
            ...
            while let Some(destroy_id) = {
                ...
                self.windows.retain(|(window, _event_state)| {
                    WindowId::from_raw(window.fd as usize) != destroy_id
                });
            }
